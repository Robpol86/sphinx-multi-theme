...
define POETRY_MIN_AWK
BEGIN {
	print("# Auto-generated by Makefile. Derived from pyproject.toml.\n")
}
...
pyproject-min.toml: export program = $(POETRY_MIN_AWK)
pyproject-min.toml: _HELP = Duplicate pyproject.toml and pin Sphinx to the lowest support version
pyproject-min.toml: pyproject.toml
	awk "$$program" $< > $@

poetry-min.lock: _HELP = Like poetry.lock but for pyproject-min.toml
poetry-min.lock: pyproject-min.toml
	set -eux && \
		TMPDIR=$$(mktemp -d) && \
		cp $< "$$TMPDIR/pyproject.toml" && \
		(cd "$$TMPDIR" && poetry lock --no-update) && \
		cp -v "$$TMPDIR/poetry.lock" $@ && \
		rm -r $$TMPDIR
...
relock:
	...
	rm -f poetry-min.lock && $(MAKE) poetry-min.lock
